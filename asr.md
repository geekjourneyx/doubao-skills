...
根据场景，选择需要访问的集群。在控制台创建应用并开通一句话识别服务后，显示的 Cluster ID 字段。
...
对于同一个请求的多个包序号，从 1 递增，最后一包取相反数。如 1,2,3 -4。
...
默认值：audio_in,resample,partition,vad,fe,decode。
...
对于同一个请求的多个包序号，从 1 递增，最后一包取相反数，例如第五包为最后一包，则该包 sequence 为 -5。
...
2. 如何开启 ITN、顺滑、标点
可通过workflow 参数，控制是否开启 ITN，标点或顺滑。
